...
> **Apple's answer-surface opt-out is `nosnippet`, and it is blunt (2026-09-21).** Apple documents two separate levers: **`Applebot-Extended`**, disallowed in `robots.txt`, refuses **training** without affecting search appearance; and for the answers themselves, *"Web publishers can opt out of their content being used in these broad world knowledge answers by applying the `nosnippet` meta tag to specific content"* ([Apple Support, updated 4 Sep 2026](https://support.apple.com/en-us/119829)). Siri AI shipped on 14 Sep 2026 (see [02 · The Engines](02-engines.md#siri-ai-apple--new-large-and-unmeasured)).
>
> **The trade-off is the one Google spent 2026 escaping.** `nosnippet` also removes your ordinary search snippets, so unlike Google's *Search generative AI* control there is **no way to leave Siri's world-knowledge answers while keeping normal snippet presentation**. Compare the three controls before advising anyone: Google's toggle is surgical, Cloudflare's **Disallow AI Training** is surgical at the `robots.txt` layer, and Apple's is a blunt instrument. For almost every reader the correct action remains **no action**.

> **Dated note (2026-09-21).** Google's crawler changelog entry of **17 Sep 2026** reads *"Generalized the Mediapartners-Google crawler documentation"* ([changelog](https://developers.google.com/crawling/docs/changelog)). A `robots.txt` rule written years ago to govern an AdSense crawler may now govern a broader and deliberately unspecified set of Google ad products — worth a re-read of your `Mediapartners-Google` lines, even though nothing about AI retrieval changed.

### The enforcement gap: robots.txt is a request, not a lock

In **August 2025**, Cloudflare publicly accused Perplexity of **"stealth crawling"** — using undeclared user-agents (posing as a normal Chrome browser on macOS) and rotating IPs/ASNs to fetch content from sites that had blocked its declared bots — by Cloudflare's own account, "millions of requests per day" across "tens of thousands of domains". Cloudflare responded by de-listing Perplexity as a *verified* bot and blocking the traffic; Perplexity denied the characterization ([Cloudflare, Aug 2025](https://blog.cloudflare.com/perplexity-is-using-stealth-undeclared-crawlers-to-evade-website-no-crawl-directives/) · [via Search Engine Journal, 2025](https://www.searchenginejournal.com/cloudflare-delists-and-blocks-perplexity-from-crawling-websites/552899/) · [Daring Fireball summary](https://daringfireball.net/linked/2025/08/05/cloudflare-perplexity)).
...
> **Correction and addition (2026-09-14).** The note above says *existing domains keep their current settings*. That is what Cloudflare's blog post says; Cloudflare's **press release of the same day** goes further: *"For new customers and for new sites for existing customers, on September 15, 2026, the defaults will be set to allow for search but block training and agent use for pages with ads"* — and *"On September 15, 2026 these changes will also be made for all existing free customers that have not changed their settings by September 15, 2026 in their dashboard."* Read the broader one: **an untouched Free-plan zone that shows ads is moved to the new defaults too.** The blog post's sentence on multi-purpose crawlers is also worth having verbatim: *"Since the defaults will be enforced by the most restrictive applicable rules, multi-purpose crawlers such as Googlebot, Applebot, and BingBot will be blocked by customers who have selected to block Training."* ([Cloudflare press release, 1 Jul 2026](https://www.cloudflare.com/press/press-releases/2026/cloudflare-allows-the-agentic-internet-to-flourish-with-a-simple-philosophy-your-content-your-rules/) · [Cloudflare blog, 1 Jul 2026](https://blog.cloudflare.com/content-independence-day-ai-options/)). As of 13 Sep 2026 Cloudflare had published no further clarification — blog, bots changelog and AI Crawl Control docs checked, see [`updates/2026-W38.md`](../updates/2026-W38.md). `⚠️ needs-verification after 15 Sep: whether Googlebot is in practice blocked on ad pages of affected zones depends on how Cloudflare classifies it on the day — check your own AI Crawl Control dashboard.`

> **Resolution (2026-09-21) — it happened, and Cloudflare shipped a way out the same day.** The `needs-verification` flag above is now settled, in Cloudflare's own words on **15 September 2026**: *"Block and Block on pages with ads now apply to mixed-use crawlers, including Applebot, Bingbot, and Googlebot, so either setting impacts search as well as training."* So yes — **if you chose *Block* to stop AI training, you are blocking Googlebot for search too.**
>
> The same post introduces the setting that resolves the trade-off: **Disallow AI Training**, which works at the `robots.txt` layer rather than by blocking at the edge. Cloudflare: it is *"named for the `Disallow:` directive it publishes in your robots.txt"*, published on your behalf by Bot Preference Sync, and under it *"Accountable mixed-use crawlers remain allowed for search. Every other training crawler is blocked."* Per vendor, as Cloudflare states it:
>
> | Vendor | Token published | Search impact |
> |---|---|---|
> | Google | `Google-Extended` | *"Google has also stated that disallowing Google-Extended does not impact search ranking."* |
> | Apple | `Applebot-Extended` | Training disallowed; search unaffected |
> | Microsoft | — none yet | *"Microsoft is extending these capabilities and currently building the mechanism to also respect a 'no training' preference in robots.txt at the domain/site level, targeted for early 2027."* The setting *"will not automatically convey a no-training preference to Bing."* |
>
> New domains are now preset by ad status: ad-supported sites default to **Disallow AI Training**, non-ad sites default to **Allow** ([Cloudflare, 15 Sep 2026](https://blog.cloudflare.com/accountable-mixed-use-ai-crawlers/)).
>
> **What to do:** if you are on Cloudflare and picked *Block* or *Block on pages with ads* in order to refuse training, switch to **Disallow AI Training** — it expresses the same preference through the vendors' own robots.txt tokens without costing you Googlebot. Plan around the one gap: **it does nothing for Bing until early 2027.**
...
- **New in 2026:** Google introduced a **Search Console toggle** to remove content from **AI Overviews and AI Mode without affecting normal rankings/snippets**, reported effective **17 June 2026** — but, as of that rollout, **available only to a subset of operators (initially UK)**, tied to regulation ([9to5Google, 2026](https://9to5google.com/2026/06/02/google-ai-mode-overviews-opt-out/) · [Search Engine Land, 2026](https://searchengineland.com/google-ai-opt-out-feature-competitors-480375)).
- **Update — the toggle went worldwide on 31 August 2026.** The geo-limit is gone. Google's documentation for the **Search generative AI control** now states: *"As of August 31, 2026, we've rolled out this control to all websites worldwide."* The control governs whether your content *"can appear in Search generative AI features, including showing up as links and helping to ground AI responses"*, covering **AI Overviews, AI Mode, and generative AI features in Discover**. Two sentences matter most for how to advise on it:
  - *"This control only affects whether your content can appear in certain Search generative AI features; this control isn't used as a ranking or inclusion signal affecting other parts of Search."* — so it is genuinely separable from Search performance, unlike `nosnippet`.
  - *"You won't receive any traffic or impressions from these features."* — opting out is total for those surfaces, not a "reduce my exposure" dial. Content may still be used to power Google Search as a whole.

  ([Google Search Console Help — *Search generative AI control*](https://support.google.com/webmasters/answer/16908024) · [Search Engine Land, 31 Aug 2026](https://searchengineland.com/google-search-console-ai-performance-reports-and-search-generative-ai-control-rolling-out-globally-486269))

  **What this changes in practice:** the historic complaint in this section — that there was *no way to opt out of AI features while keeping a normal snippet* — no longer holds anywhere in the world. For the overwhelming majority of GEO readers the right action is still **no action**, because the goal is to *be* cited; but if a publisher genuinely wants out of AI answers, the blunt `nosnippet` trade-off is no longer the only route.
...
> ⚠️ **As of 2026-08, no major AI engine has publicly confirmed that it reads or acts on `llms.txt` for retrieval, ranking, or citation.** Google's Search team (via public comments from **Gary Illyes / John Mueller**) has said it does **not** use or support it. Google's *written* documentation stops short of that claim, but it is the primary, citable source for the narrower point that matters in practice — that no such file is **needed** to appear in Google's own AI features (**AI Overviews and AI Mode**): *"You don't need to create new machine readable files, AI text files, or markup to appear in these features. There's also no special schema.org structured data that you need to add."* ([Google Search Central, *AI features and your website*, updated 2025-12-10](https://developers.google.com/search/docs/appearance/ai-features)). Read that limit precisely: it states non-necessity, not non-use — Google plainly *does* use schema.org, for classic rich results — and it is scoped to Google's own AI surfaces, not to every engine's retrieval. OpenAI, Anthropic, and Perplexity do **not** document it as a visibility or citation signal.

The evidence:

- **The first server-log measurement, and what it actually shows (2026-09-17).** Nathan Hall, *"Discovery, not adoption: crawler retrieval of an `llms.txt` resource announced through `robots.txt` …"* ([10.5281/zenodo.22814844](https://doi.org/10.5281/zenodo.22814844), preprint, CC BY 4.0), measured a narrower question than the usual one: what happens when the file's URL is **announced** in a `robots.txt` `Sitemap:` line. Across six WordPress sites and **165 announced site-days**, crawlers requested `/ai/llms.txt` **1,549 times (9.4 per site-day, 95% interval 8.9–9.9)** — while **ten un-referenced files in the same directory got zero requests across 60 file-months**, the **un-announced root `/llms.txt` on all seven sites was requested twice**, and a seventh site carrying only a root file logged **373 passing crawler requests and none for `llms.txt`**. **ClaudeBot** alone made **1,131** requests, a median **8 seconds** after its most recent same-site `robots.txt` fetch. The author's own reading is the one to quote: *"Tell a crawler where a file is and it fetches the file. That is discovery, not adoption."* He states the ceiling explicitly — the results *"do not establish recognition of llms.txt, parsing, indexing, ranking, or use in AI-generated answers."* `⚠️ Declared competing interest, in the deposit's words: "the author owns Lefty Media Co and builds the plugin that announces the file measured here." Treat as a well-documented single-operator result awaiting independent replication.`
- **Google's John Mueller supplied the confound in public — and it is the practical lesson here.** Replying to that study on **17 Sep 2026**: *"There are a bunch of 'everything llms.txt' sites out there now that try to check all domains for the llms.txt file and link to it from their site. So when a crawler sees their site, they see the links, and follow them."* The author agreed and gave the split for his root file: *"2 verified crawler fetches in 31 days and 565 from scanners and SEO tools"* ([Bluesky, 17 Sep 2026](https://bsky.app/profile/johnmu.com/post/3mvqdmbyeuk2u) · [reply, 18 Sep 2026](https://bsky.app/profile/leftymediaco.bsky.social/post/3mvspl3bavc22)). **So: a raw hit count on `/llms.txt` is close to worthless as evidence of engine interest** — most of it is directory sites and audit tools that exist *because* of the `llms.txt` discourse. Verify the user-agent against the vendor's published IP ranges, or you are measuring the hype rather than the engines. Mueller's older and blunter line, same thread, **17 Jun 2025**: *"FWIW no AI system currently uses llms.txt."*
