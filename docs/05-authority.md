...
> **New owned entity surface (added 2026-09-21): Google Search profiles.** Google *"lowered the eligibility threshold to 10,000 followers across YouTube, Instagram, X, or TikTok"*, added management of multiple brand profiles under one login, and improved article thumbnails and headline length — bringing the feature within reach of mid-sized independent publishers and creators in the US ([Google, 16 Sep 2026](https://blog.google/products-and-platforms/products/search/3-new-ways-were-improving-search-profiles-for-publishers/)). Google's documentation describes it as a consolidation surface: *"A Search profile brings together your content from across the web and social platforms into a single destination on Google"* ([Search Central](https://developers.google.com/search/docs/appearance/search-profiles)). That is exactly the *disambiguated, owned, verified* entity node this section argues for, and it is now cheap to obtain.
>
> `⚠️ Do not over-read it. Google's announcement does not mention AI Mode, AI Overviews or citations at all. Any claim that a Search profile improves AI-answer inclusion is currently unevidenced — treat it as entity hygiene with a plausible but unmeasured downstream effect, and note that there are now two Google-issued embeddable buttons (Preferred Sources and Search profile) competing for the same header space.`
