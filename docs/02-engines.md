...
| `Google-Extended` | Google | Opt-out **token** (not a distinct fetcher) for Gemini/Vertex training & grounding | Yes | Content withheld from Gemini training/grounding — **does not affect Search ranking** | [Google Search Central](https://developers.google.com/search/docs/appearance/ai-features) |
...
- **`Google-Extended`** opts you out of Gemini training/grounding **without affecting your Google Search ranking or inclusion** — a genuinely separate lever from Search visibility.
