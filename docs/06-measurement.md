...
Both of these change the *instrument*, not the sites being measured, and both fall inside 13–20 September 2026. Any series that crosses this window needs them recorded in its methodology or it cannot be read.

- **Google tightened scraper blocking around 13 Sep, and third-party SERP datasets thinned.** Nozzle's Derek Perkins reported roughly an **80% drop** in collectable data; Sistrix stated *"our data collection is currently running at a reduced rate"*; DataForSEO also reported reductions, while Ahrefs and Semrush were described as still collecting, Semrush with lag ([Search Engine Roundtable, 18 Sep 2026](https://www.seroundtable.com/google-blocking-scrapers-and-tracking-tools-42118.html); the underlying vendor statements originate on X and LinkedIn and were not independently retrieved). **A large share of AI-visibility tooling scrapes SERPs and AI Overview citation sets** — so a drop in *collection rate* will look exactly like a drop in *visibility*. Report your collection rate alongside your result, or the result is uninterpretable.
- **OpenAI stopped auto-escalating paid answers to reasoning on 14 Sep.** *"We're retiring automatic switching from Instant to Thinking (reasoning) for ChatGPT Plus and Pro users globally"* ([ChatGPT Release Notes](https://help.openai.com/en/articles/6825453-chatgpt-release-notes); 403 to automated fetchers, read from the Internet Archive raw mirror). Reasoning depth drives how many sub-queries fire and how many sources get fetched, so **a panel running on Plus/Pro accounts may record fewer sources per answer from 14 September for purely configuration reasons.**

> **The general rule these two make concrete.** This is the third month running in which a *vendor configuration change* — the Gemini 3.8 Flash citation regression (2–4 Sep, see [`updates/2026-W38.md`](../updates/2026-W38.md)), now these two — would have been misread as a visibility movement by a panel that did not record its own conditions. **Every citation measurement must log which engine, which model, which mode, which account tier, and what fraction of intended samples were actually collected.** Without those five fields a time series is not evidence.
