...
Publish a version when the change **reaches a reader**: revised chapters, a corrected or withdrawn claim, a
`needs verification` marker resolved against a primary source, a new weekly landscape update worth citing.
Continuous integration, internal tooling, formatting and dependency pins do not reach a reader and do not justify
a version.
...
One pull request that moves every version string at once. In this repository the version lives in five places:
...
| `llms.txt` | the `- Version:` line |
| `CHANGELOG.md` | promote `## [Unreleased]` to `## [X.Y.Z] — YYYY-MM-DD` and open a fresh `## [Unreleased]` |

`llms.txt` is the one that gets forgotten, because nothing fails when it is stale — it is served to machines, and
a machine reading a version that no longer exists is exactly the failure this repository is about.
...
`claims`, `Link check` and `translations` all have to be green on `main`. Do not tag a commit whose checks have
not completed.

### 3. Create the tag on the merge commit

```
gh api -X POST repos/ferinazumaDEV/generative-engine-optimization-handbook/git/refs \
...
```
gh release create vX.Y.Z -R ferinazumaDEV/generative-engine-optimization-handbook \
...
rid = "22299644"   # concept record id for this work
...
(`10.5281/zenodo.22299644`), which is what `CITATION.cff` carries, stays the same forever and always resolves to
...
```
curl -s -X POST "https://archive.softwareheritage.org/api/1/origin/save/git/url/https://github.com/ferinazumaDEV/generative-engine-optimization-handbook/"
```
...
- **Never quietly rewrite a claim that has been cited.** Correct it in place *and* say in the changelog what it
  used to say and why it was wrong. A claim that changes without a record is worse than one that was wrong.
