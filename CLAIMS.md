...
| 6 | Client-side-only rendering is invisible to a fetch-only crawler (6 → 152 words) | `established` for the property · engine effect `experimental` | yes — Cookbook `ssr-vs-csr-rendering` | [`04-technical/ssr-vs-csr-rendering`](https://github.com/ferinazumaDEV/generative-engine-optimization-cookbook/tree/main/04-technical/ssr-vs-csr-rendering) — the recipe's `reproduce.sh` re-derives the number offline | One controlled page pair; no engine's citation behaviour was measured. |
| 7 | JSON-LD makes typed facts extractable (0 → 37 facts, 0 → 17 entities) | `established` for the property · engine effect `experimental` | yes — Cookbook `structured-data-jsonld` | [`04-technical/structured-data-jsonld`](https://github.com/ferinazumaDEV/generative-engine-optimization-cookbook/tree/main/04-technical/structured-data-jsonld) — the recipe's `reproduce.sh` re-derives the number offline | Same as 6. |
| 8 | Allowing AI-related robots.txt tokens and publishing `llms.txt` declares content reachable (0 → 8 of 8 tokens allowed; 0 → 2,868 bytes) | `established` for the precondition · engine effect `experimental` | yes — Cookbook `ai-crawler-access` | [`04-technical/ai-crawler-access`](https://github.com/ferinazumaDEV/generative-engine-optimization-cookbook/tree/main/04-technical/ai-crawler-access) — the recipe's `reproduce.sh` re-derives the number offline | Declared access is a precondition of citation, not a citation. The 8 tokens mix retrieval, training and usage-control tokens; not all are HTTP crawlers (see the recipe's `meta.yml`). |
| 9 | Self-contained sections survive a fixed-size splitter (0 of 5 → 5 of 5) | `established` for the property · engine effect `experimental` | yes — Cookbook `chunk-friendly-structure` | [`03-content/chunk-friendly-structure`](https://github.com/ferinazumaDEV/generative-engine-optimization-cookbook/tree/main/03-content/chunk-friendly-structure) — the recipe's `reproduce.sh` re-derives the number offline | One article, one splitter setting (`chunk_size = 800`). |
| 10 | `sameAs` / `@id` resolve named entities to one Wikidata ID (0 → 5 of 5) | `established` for the property · engine effect `experimental` | yes — Cookbook `entity-clarity-sameas` | [`05-authority/entity-clarity-sameas`](https://github.com/ferinazumaDEV/generative-engine-optimization-cookbook/tree/main/05-authority/entity-clarity-sameas) — the recipe's `reproduce.sh` re-derives the number offline | Five entities on one page. |
| 11 | Inline sources yield parser-liftable claim→source pairs (0 → 8 of 8) | `established` for the property · engine effect `experimental` | yes — Cookbook `citation-anchoring` | [`06-measurement/citation-anchoring`](https://github.com/ferinazumaDEV/generative-engine-optimization-cookbook/tree/main/06-measurement/citation-anchoring) — the recipe's `reproduce.sh` re-derives the number offline | Eight claims in one document; the recipe rates its own confidence low–moderate. |
| 12 | Answers from live AI search vary across runs, prompts and time, so a one-off observation is not a representative measurement | `established` (as stated in the paper's abstract, which draws on its empirical studies) | no | Schulte, Bleeker & Kaufmann, 2026 — [arXiv:2604.07585](https://arxiv.org/abs/2604.07585) | The Jaccard range quoted in the Cookbook changelog (≈ 0.32–0.43) is repeated from that paper and carries `⚠️ needs verification` here until re-read against its tables; the direction is why a small live test cannot show a lift. — Re-read 2026-09-23 on the HTML version: Table 4 gives repeated-run Jaccard of 0.321–0.434 within 24 hours, stated in the text as "between 0.32 and 0.43", so the range is confirmed (Table 2's day-to-day range is 0.336–0.423). |
